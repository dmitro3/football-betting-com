...
export const getOne = async (req: Request, res: Response) => {
    const result = await BracketsBets.aggregate([{ $match: { _id: ObjectId(req.params.id) } }, ...aggregateQuery]);
...
export const list = async (req: Request, res: Response) => {
    ...
    if (!pageSize || !page) {
        const results = await BracketsBets.aggregate([{ $match: query }, ...aggregateQuery, { $sort: sortQuery }]);
...
export const updateOne = async (req: Request, res: Response) => {
    ...
    const result = await BracketsBets.aggregate([{ $match: query }, ...aggregateQuery]);
    return res.json(result[0]);
};
...
export const deleteOne = async (req: Request, res: Response) => {
    const result = await BracketsBets.deleteOne({
