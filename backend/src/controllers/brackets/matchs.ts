...
export const getOne = async (req: Request, res: Response) => {
    const result = await BracketsMatchs.aggregate([{ $match: { _id: ObjectId(req.params.id) } }, ...aggregateQuery]);
    return res.json(result[0]);
};

export const list = async (req: Request, res: Response) => {
    const { sort = null, column = null, date = null, page = null, pageSize = null } = req.body;
    let query = {} as any;
    ...
    if (!pageSize || !page) {
        const results = await BracketsMatchs.aggregate([{ $match: query }, ...aggregateQuery, { $sort: sortQuery }]);
...
export const update = async (req: Request, res: Response) => {
    ...
    const result = await BracketsMatchs.aggregate([{ $match: query }, ...aggregateQuery]);
    return res.json(result[0]);
};
