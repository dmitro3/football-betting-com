...
import { Balances, Currencies, BracketsBets, BracketsMatchs } from '../../models';
...
export const betBracket = async (req: Request, res: Response) => {
    ...
    const match = await BracketsMatchs.findById(ObjectId(matchId));
    if (Date.now() > new Date(match.time).getTime())
        return res.status(400).json('The bracket can no longer be submitted and has already been started.');
    const currency = await Currencies.findOne({
        symbol: process.env.DEFAULT_CURRENCY
    });
    const balance = await Balances.findOne({ userId, currency: currency._id });
    if (balance) {
        const amount = 200 / currency.price;
        if (balance.balance < amount) {
            return res.status(400).json(`${process.env.DEFAULT_CURRENCY} is not enough.`);
        } else {
        ...
        return res.status(400).json(`Please deposit ${process.env.DEFAULT_CURRENCY}.`);
    }
};
...
export const updateBracketPoint = async (id: string) => {
    ...
    if (!matchs) return;
    const bets = await BracketsBets.find();
    let result = {} as any;
    const points = [
        ...
        for (const j in bracket) {
            for (let k = 1; k < 6; k++) {
                if (bracket[j].round === k + 1) {
                    const match = matchs.events.find((e: any) => e.round === k + 1 && e.pid === bracket[j].pid);
                    const point = points.find((e) => e.key === k) as any;
                    if (match.team1?.name === bracket[j].team1?.name && match.team1?.id === bracket[j].team1?.id) {
                        result[bets[i]._id] = result[bets[i]._id] + point.value;
                    }
                    if (match.team2?.name === bracket[j].team2?.name && match.team2?.id === bracket[j].team2?.id) {
                        result[bets[i]._id] = result[bets[i]._id] + point.value;
                    }
                }
            }
