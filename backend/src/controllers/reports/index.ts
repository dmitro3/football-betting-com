...
const getChartData = async (qdate: any) => {
    ...
    const cPayments = await Payments.aggregate([{ $match: { $and: [{ status: { $ne: 0 } }, { status: { $ne: -1 } }] } }, ...query]);
    ...
    charts.sort((a: any, b: any) => new Date(a.name).valueOf() - new Date(b.name).valueOf());
    return charts;
};
...
const getCasinoBetData = async (qdate: any) => {
    ...
    const data = [] as any;
    for (const i in currencies) {
        const sportsbet = games.filter((e) => e.currency === currencies[i].currency);
        let win = 0;
        let lost = 0;
        let refund = 0;
        let profit = 0;
...
export const getCProfit = async (req: Request, res: Response) => {
    ...
    const result3 = await Games.aggregate([
        ...
        },
        {
            $group: {
                ...
                profit: {
                    $sum: {
                        $subtract: [{ $multiply: ['$amount', '$currency.price'] }, { $multiply: ['$profit', '$currency.price'] }]
                    }
                }
            }
        },
    ...
    for (const key in dates) {
        ...
        if (win || lost || profits) {
            const total = (win?.total ? win.total : 0) + (lost?.total ? lost.total : 0);
            const profit = (win?.profit ? win.profit : 0) + (lost?.profit ? lost.profit * -1 : 0) + (profits?.profit ? profits.profit : 0);
