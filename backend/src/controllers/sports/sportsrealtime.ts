...
const getEvents2 = async () => {
    const sportslist = await SportsLists.find({ status: true });
    for (const key in sportslist) {
        const sportsLeagues = await SportsLeagues.find({
            status: true
        });
        console.log(sportsLeagues.length, '==>sportsLeagues');
        sportsLeagues.map(async (item) => {
            await getUpcomingPage(sportslist[key].SportId, item.id, moment().format('YYYYMMDD'));
        });
    }
};
...
export const getEnds = async () => {
    // const sportsmatchs = await SportsBetting.aggregate([{ $match: { status: 'BET' } }, { $group: { _id: '$eventId' } }]);
    const sportsmatchs = await BetRooms.aggregate([{ $match: { finished: false } }, { $group: { _id: '$eventId' } }]);
...
const getUpcomingPage = (sport_id: number, league_id: string, day: string) => {
    return new Promise(async (resolve, reject) => {
        setTimeout(() => {
            resolve('success');
        }, 1000);
        const options = {
            ...
            qs: { token, sport_id, league_id: league_id, skip_esports: 'Esports' },
            headers: { 'Content-Type': 'application/json' },
...
const getUpcomingEvents = (sport_id: number, league_id: string, page: number, day: string) => {
    const options = {
        method: 'GET',
        url: process.env.PRE_ENDPOINT as string,
        headers: { 'Content-Type': 'application/json' },
