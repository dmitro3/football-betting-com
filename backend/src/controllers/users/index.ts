...
import { Users, Sessions, Permissions, Balances, Currencies, LoginHistories, BalanceHistories } from '../../models';
import {
    signAccessToken,
    ObjectId,
    getIPAddress,
...
export const signup = async (req: Request, res: Response) => {
    ...
    if (!u_result || !b_result) {
        ...
        return res.json('You have successfully created in as a user to WCI bets.');
    }
};
...
export const signinSolana = async (req: Request, res: Response) => {
    const { signature, publicAddress } = req.body;
    const user = await Users.findOne({
        publicAddress: {
            $regex: new RegExp('^' + publicAddress.toLowerCase(), 'i')
