...
const DepositMetamask = forwardRef(({ modalStyle, functions }: Props, ref: React.Ref<HTMLDivElement>) => {
    ...
    const depositMetamask = (txn_id: string, amounti: BigNumber) => {
        Api.depositMetamask({
            ...
            .then(({ data }) => {
                ...
                snackbar(formatMessage({ id: 'success.' }), 'success');
            })
            .catch(() => {
                setLoading(false);
            });
    ...
    const onDepositMetamask = async () => {
        if (Number(currency.minDeposit) > Number(amount)) {
            snackbar(formatMessage({ id: `Minimum Deposit ${currency.minDeposit} ${currency.symbol}` }), 'error');
            return;
        }
        if (!account) {
